...
-- if both previous trains, add a second
	if event.old_train_id_2 and listTrains[event.old_train_id_2] then
		for i,d in pairs(listTrains[event.old_train_id_2].globalData.guiData.signals or {}) do
			table.insert(listTrains[event.train.id].globalData.guiData.signals,{signal=d.signal,count=d.count,index=#listTrains[event.train.id].globalData.guiData.signals})
		end
