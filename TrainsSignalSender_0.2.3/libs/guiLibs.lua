...
function OnAction(guiElement,eventType)
	...
	if MappingGuiObject[path] and MappingGuiObject[path].luaGuiObject.onAction then
		MappingGuiObject[path].luaGuiObject.onAction(MappingGuiObject[path],eventType)
	end
end
...
function LuaGuiObject:updateGuiElement(data)
	...
	elseif self.gui.type=="drop-down" then
		local i=0
		while (i<#(self.gui.items) and self.gui.selected_index==0) do 
			if self.gui.items[i]==data[self.content] then self.gui.selected_index=i end i=i+1 
		end		
...
function LuaGuiObject:updateData(guiElement,data)
	...
	elseif guiElement.type=="slider" then
		data[self.content]=guiElement.slider_value
...
function LuaGuiObject:addToGui(parent,data,rootData)
	...
	if self.content and data[self.content] then
